...
Public Class TestCapDlg
    ...
    Public Sub New(ByVal item As System.IntPtr, theCapToTest As String)
        ...
        capToTestAsInt = DTWAINAPI.DTWAIN_GetCapFromName(capToTest)
    End Sub
    ...
    Private Sub SetTestSelection(ByVal typeOfGet As String)
        ...
        Dim nID As Integer = DTWAINAPI.DTWAIN_GetConstantFromTwainName(typeOfGet)

        ' Choose the container type for the capability 
        Dim bestContainer As Integer = DTWAINAPI.DTWAIN_GetCapContainer(m_Source, capToTestAsInt, nID)
        ...
        ' Get the data type 
        Dim bestDataType As Integer = DTWAINAPI.DTWAIN_GetCapDataType(m_Source, capToTestAsInt)

        ' Get the name of the data type
        Dim szBestDataType As StringBuilder = New StringBuilder(100)
        DTWAINAPI.DTWAIN_GetTwainNameFromConstant(DTWAINAPI.DTWAIN_CONSTANT_TWTY, bestDataType, szBestDataType, 100)
    ...
    Private Sub SetTestSelection2(setType As String)
        ...
        If DTWAINAPI.DTWAIN_GetCapOperations(m_Source, capToTestAsInt, capOpts) = 1 Then
        ...
        Dim nID As Integer = DTWAINAPI.DTWAIN_GetConstantFromTwainName(setType)
    ...
    Private Sub TestSetCap()
        ...
        Dim nSetType As Integer = DTWAINAPI.DTWAIN_GetConstantFromTwainName(szSetType)
        ...
        Dim nDataType As Integer = DTWAINAPI.DTWAIN_GetConstantFromTwainName(szDataType)
        ' Get the input 
        ...
        If arrayType = DTWAINAPI.DTWAIN_ARRAYFRAME Then
            ...
            Next i
            aValues = DTWAINAPI.DTWAIN_FrameCreate(frameValues(0), frameValues(1), frameValues(2),
                                                               frameValues(3))
        Else
            aValues = DTWAINAPI.DTWAIN_ArrayCreate(arrayType, 0)
            If arrayType = DTWAINAPI.DTWAIN_ARRAYANSISTRING Then
                DTWAINAPI.DTWAIN_ArrayAddANSIString(aValues, szInput)
            Else
                Select Case nDataType
                    Case DTWAINAPI.DTWAIN_TWTY_BOOL
                        Dim value As Integer = BoolStringToInt(items(0))
                        DTWAINAPI.DTWAIN_ArrayAddLong(aValues, value)
                    ...
                    Case DTWAINAPI.DTWAIN_TWTY_INT8,
                        ...
                        Dim value As Integer = DTWAINAPI.DTWAIN_GetConstantFromTwainName(items(0))
                        If value <> Integer.MinValue Then
                            DTWAINAPI.DTWAIN_ArrayAddLong(aValues, value)
                        ElseIf String.IsNullOrWhiteSpace(items(0)) Then
                            DTWAINAPI.DTWAIN_ArrayAddLong(aValues, 0)
                        Else
                            DTWAINAPI.DTWAIN_ArrayAddLong(aValues, SafeToInt32(items(0)))
                        End If
                    Case DTWAINAPI.DTWAIN_TWTY_FIX32
                        DTWAINAPI.DTWAIN_ArrayAddFloat(aValues, SafeToDouble(items(0)))
                End Select
            End If
        End If
        ' Call the capability function 
        Dim ret As Integer = DTWAINAPI.DTWAIN_SetCapValuesEx2(m_Source, capToTestAsInt, nSetType, nContainerType,
                                                    nDataType, aValues)
        Dim lastError As Integer = DTWAINAPI.DTWAIN_GetLastError()
        DTWAINAPI.DTWAIN_ArrayDestroy(aValues)
        If ret = 1 Then
            lstResultsSet.Items.Add("Ok")
        Else
            Dim szErr As StringBuilder = New StringBuilder(8192)
            DTWAINAPI.DTWAIN_GetErrorString(lastError, szErr, 8192)
    ...
    Private Sub TestGetCap()
        ...
        Dim nGetType As Integer = DTWAINAPI.DTWAIN_GetConstantFromTwainName(szGetType)
        ...
        If Not bIsCapNameSupported And Not bIsCustomCap Then
            ...
            Dim nValue As Integer = DTWAINAPI.DTWAIN_GetTwainNameFromConstant(DTWAINAPI.DTWAIN_CONSTANT_CAPCODE_NOMNEMONIC,
            ...
            If bTranslate = 1 Then
                ...
                nTranslationID = -1
                bGotID = DTWAINAPI.DTWAIN_GetTwainNameFromConstant(DTWAINAPI.DTWAIN_CONSTANT_CAPCODE_MAP, capToTestAsInt, szID, 100)
                If bGotID > 0 Then
        ...
        Dim ret As Integer = DTWAINAPI.DTWAIN_GetCapValuesEx2(m_Source, capToTestAsInt, nGetType, nContainerType, nDataType, values)
        If ret = 1 Then
            lblTestGetResults.Text = "Success"
            Dim szValues As StringBuilder = New StringBuilder(1024)

            ' Display the results in the list box 
            Dim numItems As Integer = DTWAINAPI.DTWAIN_ArrayGetCount(values)
            Dim nArrayType As Integer = DTWAINAPI.DTWAIN_ArrayGetType(values)
            For i As Integer = 0 To numItems - 1
                ...
                Select Case nArrayType
                    ...
                    Case DTWAINAPI.DTWAIN_ARRAYLONG
                        ...
                        DTWAINAPI.DTWAIN_ArrayGetAtLong(values, i, lVal)
                        ...
                        If capToTestAsInt = DTWAINAPI.DTWAIN_CV_CAPSUPPORTEDDATS Then
                            ...
                            DTWAINAPI.DTWAIN_GetTwainNameFromConstant(DTWAINAPI.DTWAIN_CONSTANT_DG, hiWord, szTemp, 30)
                            DTWAINAPI.DTWAIN_GetTwainNameFromConstant(DTWAINAPI.DTWAIN_CONSTANT_DAT, loWord, szTemp2, 30)
                            szValues.AppendFormat("{0} / {1}", szTemp.ToString(), szTemp2.ToString())
                        Else
                            If bIsCapNameSupported Then
                                DTWAINAPI.DTWAIN_GetNameFromCap(lVal, szValues, 256)
                            ...
                            Else
                                ...
                                ' Not the value
                                If bGotID = 1 Then
                                    DTWAINAPI.DTWAIN_GetTwainNameFromConstant(nTranslationID, lVal, szValues, 256)
                                Else
                    ...
                    Case DTWAINAPI.DTWAIN_ARRAYFLOAT
                        ...
                        DTWAINAPI.DTWAIN_ArrayGetAtFloat(values, i, dVal)
                        Dim sPrefix As String = ""
                        ' Check if this Is a range.  If so, there will always be 5 values
                        ' where item i Is either Minimum, Maximum, Step, Default, Or Current
                        Dim isRange As Boolean = (cmbContainer.SelectedItem.ToString() = "TW_RANGE")
                    ...
                    Case DTWAINAPI.DTWAIN_ARRAYANSISTRING
                        DTWAINAPI.DTWAIN_ArrayGetAtANSIString(values, i, szValues)
                        lstResults.Items.Add(szValues.ToString())

                    Case DTWAINAPI.DTWAIN_ARRAYFRAME
                        Dim Left, Top, Right, Bottom As Double
                        DTWAINAPI.DTWAIN_ArrayGetAtFrame(values, i, Left, Top, Right, Bottom)
                        szValues.AppendFormat("Left: {0}  Top: {1}  Right: {2}  Bottom: {3}",
                                                Left, Top, Right, Bottom)
                        lstResults.Items.Add(szValues.ToString())
                End Select
            Next i
            DTWAINAPI.DTWAIN_ArrayDestroy(values)
        Else
